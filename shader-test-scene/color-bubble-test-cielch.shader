...
uniform bool use_screen_blending;
...
void fragment() {
    ...
    // create color by taking the luminocity of the screen color and changing
    // the chroma and hue
    else {
        vec4 color = textureLod(SCREEN_TEXTURE, SCREEN_UV, 0.0);
        float l = cielab_to_cielch(xyz_to_cielab(srgb_to_xyz(color.xyz))).x;

        // COLOR = vec4(l / 100.0, l / 100.0, l / 100.0, uniform_a);

        vec3 lch = vec3(l, uniform_c, uniform_h);
        vec3 srgb = xyz_to_srgb(cielab_to_xyz(cielch_to_cielab(lch)));

        COLOR = vec4(srgb, uniform_a);
    }
    // COLOR = color;
